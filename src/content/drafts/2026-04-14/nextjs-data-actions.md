...
| 主題 | 核心機制 |
|---|---|
| Server Component 資料獲取 | 直接 `async/await`，不需 `useEffect` |
| `NEXT_PUBLIC_` 前綴 | 變數會打包進瀏覽器 JS，勿存放機密 |
| Server Action | 加 `"use server"`，前端直接觸發後端寫入 |
| `revalidatePath()` | 讓指定路由的快取失效，觸發重新 fetch |